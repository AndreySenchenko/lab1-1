...
void aboutA() {
    ...
    cout << "Група К-16" << " " << "Сенченко Андрій" << endl;
...
int whereP(double x, double y, double a) {
    ...
    if ( 1 < 2 ) return 1;     //  TODO: Need to change expression (точка за межами контуру)
    if ( 1 > 2 ) return -1;    //  TODO: Need to change expression  (точка всередині контуру)
    
    return 0; //точка на контурі
}
